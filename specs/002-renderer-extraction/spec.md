...
- `src/themes.ts` — dark/light palette definitions (background, star fills, comet head/trail, labels).
...
- Three output modes per `animated: boolean` × `theme: 'dark' | 'light'` combinations plus a static `reduced-motion` variant — total seven SVG artifacts generated by the same function with different option flags (one per theme, plus reduced fallback covered by `animated: false`).
