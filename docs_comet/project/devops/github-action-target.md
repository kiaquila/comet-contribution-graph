...
- **Action implementation — in progress.** PR 002 (renderer extraction) is the active work item.
...
| 005 | `specs/005-dogfood/` (created when PR 004 merges)                         | S    | `.github/workflows/comet-graph.yml` (weekly cron + `workflow_dispatch`), README overhaul with `<picture>` embed, usage docs for external users.                                                 |
...
| Output branch name            | `comet-graph`                                 | Dedicated orphan branch, force-pushed each run. README refs `raw.githubusercontent.com/USER/USER/comet-graph/comet-{light,dark,reduced}.svg`. |
...
Two SVG files per run: `comet-light.svg`, `comet-dark.svg`. README embed uses the official GitHub Markdown `<picture>` pattern:

```markdown
<picture>
  <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/USER/USER/comet-graph/comet-dark.svg" />
  <img alt="cinematic comet contribution graph" src="https://raw.githubusercontent.com/USER/USER/comet-graph/comet-light.svg" />
</picture>
```

A third artifact — `comet-reduced.svg` — is a static variant (no `<animate>` elements) for users who want a `prefers-reduced-motion` fallback via a second `<source media="(prefers-reduced-motion: reduce)">` inside their `<picture>`.
...
| `theme`    | no       | `both`                | `dark`, `light`, or `both` (writes two files)         |
| `branch`   | no       | `comet-graph`         | Output branch                                         |

`top_n`, `seed`, animation timing inputs are intentionally NOT exposed in MVP — hardcoded per fixed-decisions table. Add inputs when a second real use case appears.
